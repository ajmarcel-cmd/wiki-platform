...
export const metadata = {
  title: 'All Pages - Wiki',
  description: 'Browse all pages in the wiki',
}

export default async function AllPagesPage({ searchParams }: AllPagesProps) {
  const { page: pageParam } = await searchParams
  const page = parseInt(pageParam || '1')
  const pagesData = await getAllPages(page, 50)

  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-4xl font-bold mb-6">All Pages</h1>
      
      <div className="mb-6 text-sm text-gray-600">
        Showing {((page - 1) * 50) + 1}–{Math.min(page * 50, pagesData.total)} of {pagesData.total} pages
      </div>
